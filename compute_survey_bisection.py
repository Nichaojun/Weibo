...
if __name__ == "__main__":
    np.random.seed(42)
    files = ['/lustre/home/2401213204/code/compute_twitte/surveydata/correlation_pooled_CGSS_new.dta',
             '/lustre/home/2401213204/code/compute_twitte/surveydata/correlation_pooled_GSS_new.dta']
    ...
    for file in files:
        ...
        for year in set(df['year']):
            ...
            if data_max - data_min != 0:
                ...
                data_normalized = np.full(data.shape, 3)

            data_normalized = data_normalized.T  # Transpose for correct box counting
            
            # Correct maximum box count: it should be the number of rows in data_normalized
            ...
            for target_box_count in range(max_box_count, 0, -1):
                ...
                print(f'Best epsilon for {file.split("/")[-1].split(".")[0]} - {year} for box count {target_box_count}: {best_eps}')
            result[1] = 1
            print(result)
            ...
            tpt = file.split('/')[-1].split('.')[0]
            tpt = f'{output_folder}/{tpt}/{int(year)}_bisection'
